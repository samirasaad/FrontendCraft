...
import type { Locale, LabId } from "@/lib/types";

export const LAB_JOB_KEYS: Record<
  LabId,
  {
    job:
      | "trackJobHtml"
...
export function LabJobVisual({
  ...
  const freezeAt = interactive ? undefined : PREVIEW_STEP[labId];
  ...
  if (labId === "html") return <HtmlScene {...props} />;
  if (labId === "css") return <CssScene {...props} />;
  if (labId === "javascript") return <JsScene {...props} />;
  if (labId === "react") return <ReactScene {...props} />;
  if (labId === "tailwind") return <TwScene {...props} />;
  if (labId === "accessibility") return <A11yScene {...props} />;
  if (labId === "seo") return <SeoScene {...props} />;
  return null;
}
