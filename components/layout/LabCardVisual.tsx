...
/** Quiet, card-sized showcase — designed for the lab picker, not the live lab. */
export function LabCardVisual({
  labId,
  muted = false,
}: {
  labId: string;
  ...
  if (labId === "html") return <HtmlVisual muted={muted} />;
  if (labId === "css") return <CssVisual muted={muted} />;
  if (labId === "javascript") return <JsVisual muted={muted} />;
  if (labId === "react") return <ReactVisual muted={muted} />;
  return null;
}
