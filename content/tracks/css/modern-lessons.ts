...
function lesson(spec: LessonSpec, index: number): LessonDraft {
  return {
    ...
    content: {
      ...
      visualHint: L(
        "Change one declaration at a time and watch the layout respond.",
        "غيّر declaration واحدة كل مرة واتفرّج على الـ layout وهو بيتجاوب.",
      ),
    },
  };
}
...
const specs: LessonSpec[] = [
  {
    ...
    paragraphs: [
      ["The cascade resolves competing declarations by origin, importance, specificity, then source order. Later code wins only when the earlier tests tie.", "الـ cascade بتحل التعارض حسب المصدر و importance و specificity وبعدها ترتيب الكود. المتأخر يكسب بس لما اللي قبله يتساووا."],
      ["Specificity is a selector weight: IDs beat classes, and classes beat element selectors. It is not a reason to keep adding IDs.", "Specificity وزن للـ selector: الـ IDs تكسب الـ classes، والـ classes تكسب أسماء العناصر. مش سبب إنك تزوّد IDs."],
      ["Keep selectors shallow and use component classes. That makes overrides intentional instead of an !important arms race.", "خلّي selectors بسيطة واستخدم component classes. كده الـ overrides تبقى مقصودة بدل حرب !important."],
    ],
  ...
  },
  {
    ...
    paragraphs: [
      ...
      ["With content-box, declared width excludes padding and border. border-box includes them, which makes component sizing predictable.", "مع content-box، الـ width المعلن مش بيشمل padding و border. border-box بيشملهم وده بيخلي المقاسات متوقعة."],
  ...
  },
  {
    ...
    paragraphs: [
      ["Pixels are useful for crisp borders and deliberate limits. rem follows the root font size, so it is a strong default for typography and spacing.", "Pixels مفيدة للحدود الدقيقة والحدود المقصودة. rem بتتبع حجم خط الـ root، فممتازة للخطوط والمسافات."],
  ...
  {
    slug: "colors-typography", tier: "beginner", icon: "Type", visualizer: "type-color-lab",
    title: ["Color & Typography", "الألوان والـ Typography"],
  ...
  },
  {
    ...
    summary: ["Understand the default layout algorithm before reaching for positioning.", "افهم layout الافتراضي قبل ما تلجأ للـ positioning."],
    paragraphs: [
      ["Block boxes normally stack and stretch across available inline space. Inline content flows inside lines and ignores width and vertical margin in familiar ways.", "الـ block boxes عادة بتترص وبتتمدد في مساحة الـ inline المتاحة. الـ inline content بتمشي جوه السطور وبتتجاهل width وvertical margin بشكل معروف."],
      ["display changes how a box participates in layout, not its semantic meaning. An inline element can become block, but it remains the same HTML element.", "display بتغير مشاركة الصندوق في layout، مش معناه. عنصر inline ممكن يبقى block لكنه يفضل نفس عنصر HTML."],
      ["Use flow for document content, flex for one axis, and grid for two axes. That choice reduces magic offsets and fragile layouts.", "استخدم flow لمحتوى المستند، flex لمحور واحد، و grid لمحورين. الاختيار ده يقلل offsets السحرية وlayouts الهشة."],
    ],
    points: [["Blocks stack by default", "الـ blocks بتترص افتراضيًا"], ["Inline boxes flow with text", "الـ inline boxes بتمشي مع النص"], ["display changes layout participation", "display بتغير المشاركة في layout"]],
  ...
  {
    slug: "backgrounds-borders", tier: "beginner", icon: "Image", visualizer: "surface-lab",
    title: ["Backgrounds & Borders", "الخلفيات والحدود"],
    summary: ["Layer color, images, radius, and borders to create clear surfaces.", "ركّب لون وصور وradius وحدود عشان تعمل surfaces واضحة."],
    paragraphs: [
      ...
      ["Borders consume space in the box model, while outline does not. Use outline for focus because it remains visible without moving nearby layout.", "الحدود بتاخد مساحة في box model، لكن outline لا. استخدم outline للـ focus عشان يفضل ظاهر من غير ما يزق layout."],
  ...
  },
  {
    ...
    paragraphs: [
      ["A flex container creates a main axis and cross axis. flex-direction chooses the main direction; alignment properties operate relative to those axes.", "Flex container بتعمل main axis و cross axis. flex-direction بتختار الاتجاه؛ وخصائص المحاذاة بتشتغل بالنسبة للمحاور دي."],
    ...
    points: [["Flex is one-dimensional", "Flex أحادي البعد"], ["justify main, align cross", "justify للمحور الرئيسي وalign للعرضي"], ["Prefer gap over child margins", "فضّل gap عن margins الأطفال"]],
  ...
  },
  {
    ...
    paragraphs: [
      ...
      ["sticky participates in flow until it reaches an inset threshold. It needs a scrolling context with enough room, and overflow ancestors can change its behavior.", "sticky بتشارك في flow لحد ما توصل inset معين. محتاجة scroll context بمساحة كفاية، وoverflow ancestors ممكن يغيّر سلوكها."],
    ],
  ...
  {
    slug: "responsive-media", tier: "intermediate", icon: "Smartphone", visualizer: "responsive-lab",
  ...
  {
    slug: "custom-properties", tier: "advanced", icon: "Palette", visualizer: "variables-lab",
  ...
  },
  {
    ...
    title: ["Transitions & Transforms", "Transitions والـ Transforms"],
    summary: ["Add focused, performant feedback to state changes.", "ضيف feedback مركز وأداؤه كويس لتغييرات الحالة."],
    paragraphs: [
      ["A transition interpolates between computed values when a property changes. Declare it on the resting state so hover, focus, and exit all animate.", "Transition بتعمل interpolation بين computed values لما property تتغير. عرّفها على resting state عشان hover وfocus والخروج كلهم يتحركوا."],
      ...
      ["Motion should communicate cause and effect, not delay work. Provide a reduced-motion path for people who get motion sickness or need calmer UI.", "الحركة لازم توضح السبب والنتيجة، مش تأخر الشغل. وفّر reduced-motion للي بيتعبوا من الحركة أو محتاجين UI أهدى."],
    ],
    points: [["Transition the resting state", "حط transition على resting state"], ["Prefer transform and opacity", "فضّل transform وopacity"], ["Keep motion purposeful", "خلّي الحركة لها هدف"]],
  ...
  },
  {
    ...
    paragraphs: [
      ["@keyframes names stages of an animation; animation applies timing, duration, iteration, direction, and fill behavior to an element.", "@keyframes بتسمي مراحل animation؛ وanimation بتطبق timing وduration وiteration وdirection وfill behavior على العنصر."],
  ...
  {
    slug: "logical-properties", tier: "advanced", icon: "Languages", visualizer: "logical-layout-lab",
    title: ["Logical Properties & RTL", "Logical Properties و RTL"],
    summary: ["Write layout rules that follow reading direction instead of hard-coded left and right.", "اكتب layout rules بتتبع اتجاه القراءة بدل left وright الثابتين."],
    paragraphs: [
      ["Inline runs in the writing direction; block runs perpendicular to it. Logical properties describe those axes, so a UI can adapt to RTL and vertical writing.", "Inline بيمشي في اتجاه الكتابة؛ وblock عمودي عليه. Logical properties بتوصف المحاور دي، فـ UI تقدر تتكيف مع RTL والكتابة العمودية."],
      ["Use margin-inline, padding-block, inset-inline-start, and border-inline-start instead of physical left/right where direction should follow language.", "استخدم margin-inline وpadding-block وinset-inline-start وborder-inline-start بدل left/right لما الاتجاه لازم يتبع اللغة."],
      ["Set dir in HTML for document direction; CSS logical properties complement it. Test mixed Arabic and English content, not just a mirrored screenshot.", "حط dir في HTML لاتجاه المستند؛ وCSS logical properties بتكمله. اختبر محتوى عربي وإنجليزي مخلوط، مش screenshot مقلوبة بس."],
    ],
