...
export const cssInsights: Record<string, ProductionInsights> = {
  "cascade-specificity": {
    underTheHood: insight(
      [
        L("Browsers parse stylesheets into the CSSOM, match selectors against the DOM, then resolve the cascade for each property before layout and paint.", "المتصفح بيحوّل `stylesheets` لـ `CSSOM`، ويطابق `selectors` مع `DOM`، وبعدها يحل `cascade` لكل `property` قبل `layout` و`paint`."),
  ...
  "flexbox-basics": {
    ...
    seo: insight(
      [L("Flex is a presentation layer. Keep the source order meaningful for crawlers, reader mode, and content extraction.", "`Flex` طبقة عرض. خلّي `source order` ذات معنى للـ `crawlers` و`reader mode` واستخراج المحتوى.")],
  ...
  "logical-properties": {
    underTheHood: insight(
      [
        L("Logical properties resolve against an element's writing-mode and direction during style calculation, mapping inline and block edges to physical edges.", "`Logical properties` بتتحل حسب `writing-mode` واتجاه العنصر وقت `style calculation`، وبتربط حواف `inline` و`block` بالحواف الفعلية."),
    ...
    accessibility: insight(
      [L("Set `lang` and `dir` in HTML so assistive technology reads content correctly; logical CSS alone cannot set language or bidirectional text behavior.", "حط `lang` و`dir` في `HTML` عشان التقنيات المساعدة تقرأ المحتوى صح؛ `CSS` المنطقية لوحدها مش بتحدد اللغة أو `bidi`.")],
    ...
    seo: insight(
      [L("Localized pages need meaningful language and direction metadata in HTML. Logical layout helps the presentation remain correct across those locales.", "الصفحات المحلية محتاجة `language` و`direction metadata` ذات معنى في `HTML`. الـ `logical layout` بتساعد العرض يفضل صح عبر اللغات.")],
      { bullets: [L("Use `hreflang` and `lang` where applicable", "استخدم `hreflang` و`lang` لما ينفع"), L("Avoid separate mirrored markup", "تجنب `markup` منفصلة معكوسة")] },
    ),
  },
};
