...
const ALL_LAB_IDS = ["html-beginner", ...SHIPPED_LABS.map((l) => l.id)] as const;
...
const UI = {
  ...
  recordHint: {
    en: "9:16 frame · record inside dashed box · Space = play/pause · ~5s per step",
    ar: "إطار 9:16 · سجّل جوّا الإطار · مسافة = تشغيل · ~٥ ثواني لكل خطوة",
  },
...
/** Fixed 16:9-ish preview + 9:16 record — stable for CapCut crop */
const SANDBOX_LAB_H = "34rem";
const SANDBOX_RECORD_W = "22.5rem";
const SANDBOX_RECORD_H = "40rem";
...
function MotionLabSandboxInner() {
  ...
  const { locale, setLocale } = useLanguage();
  const ar = locale === "ar";
  const [lab, setLab] = useState<LabId>("html-beginner");
  const [recordMode, setRecordMode] = useState(false);
  const [cleanFrame, setCleanFrame] = useState(true);
  const [labEpoch, setLabEpoch] = useState(0);
  const { playing, toggle, setPlaying } = useAutoPlay(true);

  useEffect(() => {
    const labParam = searchParams.get("lab");
    if (isLabId(labParam)) setLab(labParam);
    const lang = searchParams.get("lang");
    if (lang === "en" || lang === "ar") setLocale(lang);
    if (searchParams.get("record") === "1") {
      setPlaying(true);
      setRecordMode(true);
      setLabEpoch((n) => n + 1);
    }
  }, [searchParams, setLocale, setPlaying]);
  ...
  if (recordMode) {
    return (
      <div
        className={`record-mode fixed inset-0 z-50 flex flex-col bg-black ${cleanFrame ? "record-mode--clean" : ""}`}
        style={
          {
            "--sandbox-lab-h": SANDBOX_RECORD_H,
            "--sandbox-lab-w": SANDBOX_RECORD_W,
          } as CSSProperties
        }
      >
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center px-2 pb-2">
            <div data-record-frame className={`${RECORD_LAB_FRAME_CLASS} record-frame`}>
              {labContent}
            </div>
          </div>

          <p className="shrink-0 px-4 pb-3 text-center text-[10px] leading-relaxed text-slate-600">
            {t("recordHint", locale)}
          </p>
      </div>
    );
  ...
  return (
    <div className="mx-auto min-h-dvh max-w-6xl px-4 py-8 sm:px-6">
      ...
      <div
        className={RECORD_LAB_FRAME_CLASS}
        style={{ "--sandbox-lab-h": SANDBOX_LAB_H } as CSSProperties}
      >
        {labContent}
      </div>
      ...
      <section className="mt-4 rounded-2xl border border-white/10 bg-slate-900/40 p-4 sm:p-5">
        ...
        <ol className="list-decimal space-y-2 ps-4 text-sm leading-relaxed text-slate-400">
          <li>
            {ar
              ? "اختار EN أو عربي من فوق — سجّل نسختين لنفس المعمل لو حابب."
              : "Pick EN or عربي above — record the same lab twice for two audiences."}
          </li>
          <li>
            {ar
          ...
          <li>
            {ar
              ? "فعّل «إطار نظيف» يخفي شريط الكود."
              : "Keep Clean frame on to hide the code strip."}
          </li>
          <li>
            {ar
